...
use crate::utils::{get_path_from_cstr, intersection, union};
...
pub extern "C" fn detect_faces(image_path: *const c_char, out_boxes: *mut FaceBox) -> c_int {
    ...
    let mut input_arr = Array4::<f32>::zeros((1, 3, 640, 640));

    for y in 0..640 {
        for x in 0..640 {
            let pixel = rgb.get_pixel(x, y);
            input_arr[[0, 0, y as usize, x as usize]] = pixel[0] as f32 / 255.0;
            input_arr[[0, 1, y as usize, x as usize]] = pixel[1] as f32 / 255.0;
            input_arr[[0, 2, y as usize, x as usize]] = pixel[2] as f32 / 255.0;
        }
    }

    let input_tensor = match TensorRef::from_array_view(input_arr.view()) {
        Ok(v) => v,
        Err(_) => return -11,
    };
